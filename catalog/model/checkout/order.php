                    ...
                    if ($this->config->get('config_redeem') == 1) {
                        if ($product['redeem'] == 1) {
                            for ($i = 0; $i < (int)$product['quantity']; $i++) {
                                $this->db->query("INSERT INTO " . DB_PREFIX . "redeem SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$order_product_id . "', code = 'ts" . substr(md5(mt_rand()), 0, 6) . "', status = '1', `redeem` = '0', date_added = NOW()");
                            }
                        }
                    }
		}
